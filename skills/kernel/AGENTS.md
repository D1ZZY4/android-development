...
1. Never run destructive or mutating commands on a connected physical device
   without the user's explicit confirmation. This includes: fastboot flash, fastboot erase, dd to any device node, or anything that writes to a partition. Building an image in the workspace is fine.
...
1. Identify kernel source layout: legacy (arch/arm64/configs/\<defconfig\>, single
   Makefile) vs GKI (split common/ kernel + separate vendor modules, build.config.* or BUILD.bazel).
...
4. Package: Image.gz-dtb (or Image / Image.lz4) + ramdisk into boot.img via
   mkbootimg/avbtool, or use AnyKernel3 for a flashable zip. See template/anykernel_notes.md.
